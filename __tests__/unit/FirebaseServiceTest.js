...
it('locks gateway lock correctly', () => {
  ...
  expect(gatewayLockData).toEqual({
    commands: [],
    devices: [],
    gatewayLock: '7ab2f',
  });
...
it('rejects locking locked data', () => {
  ...
  expect(() => FirebaseService.gatewayLock(data, true)).toThrow(
    Error('Can not acquire gateway lock')
  );
});
...
it('enqueues command correctly', () => {
  const data = {
    commands: [{ command: 'temp_corr', value: 5, ips: ['192.168.100.1'] }],
    devices: [],
    gatewayLock: null,
  };
  const command = {
  ...
  expect(FirebaseService.enqueue(data, command)).toEqual({
    commands: [
      { command: 'temp_corr', value: 5, ips: ['192.168.100.1'] },
      { command: 'humidity_corr', value: 5, ips: ['192.168.100.1'] },
    ],
    devices: [],
    gatewayLock: null,
  });
...
it('uploads readings without exception', async () => {
  ...
  await expect(
    FirebaseService.uploadReadings(updateDevices)
  ).resolves.toEqual();
});
